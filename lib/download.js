...
function downloadGTFS(agency_key, callback){
  ...
  async.series([
    ...
    if(err){
      ...
      console.log('Completed ' + agency_key);
      callback();
    }
  });
      ...
      if (!err){
        ...
        var child = exec(unzip, function(err, stdout, stderr) {
          if(!err){
            ...
            callback('Unzip Error' + err, 'download');
          }
        });
      
      } else { 
        callback('Download Error' + err, 'download');
      }
    });
  }
  ...
  function saveLineToDB(line, fileName){
    ...
    switch(fileName){
      ...
      case 'stops.txt':
        instance = new Stop();
        line.loc = [ parseFloat(line.stop_lat), parseFloat(line.stop_lon) ];
