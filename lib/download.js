...
function downloadGTFS(task, cb){
  ...
  function downloadFiles(cb){
    ...
    var wget = 'mkdir -p ' + downloadDir + '; wget -N -O ' + downloadDir + 'latest.zip ' + file_url;
    var child = exec(wget, function(e, stdout, stderr) {
      if (!e){
        console.log(agency_key + ': Download successful');
      
        //remove old text files and unzip file
        var unzip = 'rm ' + downloadDir + '*.txt; unzip ' + downloadDir + 'latest.zip -d ' + downloadDir;
        var child = exec(unzip, function(e, stdout, stderr) {
          if(!e){
            console.log(agency_key + ': Unzip successful');
          
            //remove old db records based on agency_key
            async.forEach(Schemas, function(schema, cb){
              schema.remove({ agency_key: agency_key }, cb);
            }, function(e){
              cb(e, 'download');
            });
        ...
        var child = exec(deleteFolder, function(e, stdout, stderr) { cb(new Error(agency_key + ': Download failed'), 'download'); });
      }
    });
  }
  ...
  function processFiles(cb){
    ...
    async.forEachSeries([
      ...
      csv()
        .fromPath(downloadDir + fileName, {columns: true})
        .on('data', function(line, index){
          if(index % 1000 == 0 && index > 0){
            console.log(agency_key + ': ' + fileName + ' - ' + index + ' lines read');
          }
      ...
      //remove GTFS downloaded file
      var deleteFolder = 'rm -rf ' + downloadDir;
      var child = exec(deleteFolder, function(e, stdout, stderr) {
