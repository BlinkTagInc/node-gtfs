    ...
    async.forEachSeries(GTFSFiles, function(GTFSFile, cb){
      ...
      exec('wc -l ' + downloadDir + GTFSFile.fileNameBase + '.csv', function(e, stdout, stderr){
        ...
        if(!lineCount || stderr){
          ...
          //break into chunks to process file
          exec('head -n 1 ' + downloadDir + GTFSFile.fileNameBase + '.csv', function(e, stdout, stderr){
            if(stdout){
              ...
              exec(moveandsplit, function(e, stdout, stderr) {
                //now put the csv headers onto each chunk, remove temporary file
                var deleteTempFile = 'rm ' + downloadDir + 'csv/' + GTFSFile.fileNameBase + '.csv;';
                exec(deleteTempFile, function(e, stdout, stderr){
          ...
          //file is small enough to be parsed by itself
          console.log(agency_key + ': ' + GTFSFile.fileNameBase + ' (' + lineCount + ' lines)');
          var move = 'mkdir ' + downloadDir + 'csv;' +
                     'rm ' + downloadDir + 'csv/*;' +
                     'mv ' + downloadDir + GTFSFile.fileNameBase + '.csv ' + downloadDir + 'csv;';
          exec(move, function(e, stdout, stderr){
            importFile(GTFSFile.collection, null, cb);
          });
        }
      })
    }, cb);
        ...
        if(files){
          async.forEachSeries(files, function(file, cb){
            ...
            if(fieldList){
               ...
               var mongoimport = 'mongoimport -d ' + dbName + ' -c ' + collection + ' --type csv --headerline --ignoreBlanks ' + downloadDir + 'csv/' + file + ';';
            }
