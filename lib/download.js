...
function downloadGTFS(task, callback){
  ...
  async.series([
    ...
    if(err){
      ...
      console.log('Completed adding ' + agency_key);
      callback();
    }
  });
  ...
  function downloadFiles(callback){
    ...
    var child = exec(wget, function(err, stdout, stderr) {
      if (!err){
        console.log('Download of ' + agency_key + ' Successful');
        ...
        var child = exec(unzip, function(err, stdout, stderr) {
          if(!err){
            ...
            callback(new Error('Unzip Error') + err, 'download');
          }
        });
      
      } else { 
        callback(new Error('Download Error') + err, 'download');
      }
    });
  }
