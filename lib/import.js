...
const processUpdates = async (task) => {
  ...
  if (updateTask.trip_updates) {
    task.log(`Downloading GTFS-RT Trip Updates File from ${updateTask.trip_updates}`);
    const tripUpdateData = await downloadRTData(updateTask.trip_updates,updateTask.update_headers);
    task.log('Download successful');

    task.log(`Processing GTFS-RT Trip Updates File from ${updateTask.trip_updates}`);
    const tripModel = models.find((x) => x.filenameBase === 'trip_updates');
    const stopModel = models.find((x) => x.filenameBase === 'stop_times_updates');

    // Await theese tasks as we do not want to delete while starting to fill
    await task.db.run(`DELETE FROM ${tripModel.filenameBase};`);
    await task.db.run(`DELETE FROM ${tripModel.filenameBase};`);

    // Loop and just dunk in to database, never mind results, next update is soon anyway
    tripUpdateData.entity.forEach((entity)=>{

      const fieldNames = tripModel.schema.map((column) => column.name);
      // eslint-disable-next-line no-eval
      const fieldValues = tripModel.schema.map((column) => eval(column.source))     
      task.db.run(`INSERT INTO ${tripModel.filenameBase} (${fieldNames.join(', ')}) VALUES (${fieldValues.join(', ')})`);

      // eslint-disable-next-line no-unused-vars
      const parent = entity;
      // eslint-disable-next-line no-unused-vars
      entity.tripUpdate.stopTimeUpdate.forEach((entity)=>{
        const fieldNames = stopModel.schema.map((column) => column.name);
        // eslint-disable-next-line no-eval
        const fieldValues = stopModel.schema.map((column) => eval(column.source))
        task.db.run( `INSERT INTO ${stopModel.filenameBase} (${fieldNames.join(', ')}) VALUES (${fieldValues.join(', ')})`);  
      });


    });     
    task.log('Processing successful');
  }
...
export async function processGtfsRt (initialConfig) {
  ...
  const agencyCount = config.agencies.length;
  log(
    `Starting GTFS-RT update for ${pluralize(
      'agency',
      agencyCount,
      true
    )} using SQLite database at ${config.sqlitePath}`
  ...
  await mapSeries(config.agencies, async (agency) => {
    ...
    if (task.rtupdates)
      await processUpdates(task);

  });

  log(`Completed GTFS-RT import for ${pluralize('agency', agencyCount, true)}\n`);
}
