...
function readFiles(task, cb) {
  ...
  log(`${task.agency_key}: Importing GTFS from ${task.path}`);
...
function importFiles(task, cb) {
  ...
  async.forEachSeries(filenames, (filename, cb) => {
    ...
    if (task.exclude && _.includes(task.exclude, filename.fileNameBase)) {
      log(`${task.agency_key}: Importing - Skipping ${filename.fileNameBase}.txt`);
      return cb();
    }
    ...
    if (!fs.existsSync(filepath)) {
      if (!filename.nonstandard) {
        log(`${task.agency_key}: Importing - No ${filename.fileNameBase}.txt file found`);
      }

      return cb();
    }

    log(`${task.agency_key}: Importing - ${filename.fileNameBase}.txt`);
    db.collection(filename.collection, (err, collection) => {
      if (err) return cb(err);
