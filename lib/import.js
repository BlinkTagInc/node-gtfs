...
const downloadRTData = async (url,headers) => {
...
function getDescendantProp(obj, desc, defaultvalue) {
  ...
  if (obj.__isLong__)
    return convertLongTimeToDate(obj)

  return obj;
}

const markRtDataStale = async (db,log) => {
  const vehiclePositionModel = models.find((x) => x.filenameBase === 'vehicle_positions');
  const tripUpdatesModel = models.find((x) => x.filenameBase === 'trip_updates');
  const stopTimesUpdatesModel = models.find((x) => x.filenameBase === 'stop_times_updates');
  const serviceAlertsModel = models.find((x) => x.filenameBase === 'service_alerts');
  const serviceAlertTargetsModel = models.find((x) => x.filenameBase === 'service_alert_targets');

  // Mark all data as stale
  log(`Marking GRFS-RT data as stale..`);
  ...
  await db.run(`UPDATE ${serviceAlertTargetsModel.filenameBase} SET isUpdated=0`);
  log(`Marked GTFS-RT data as stale\r`, true);
}

const cleanStaleRtData = async (db,log) => {
  const vehiclePositionModel = models.find((x) => x.filenameBase === 'vehicle_positions');
  const tripUpdatesModel = models.find((x) => x.filenameBase === 'trip_updates');
  const stopTimesUpdatesModel = models.find((x) => x.filenameBase === 'stop_times_updates');
  const serviceAlertsModel = models.find((x) => x.filenameBase === 'service_alerts');
  ...
  await db.run(`DELETE FROM ${vehiclePositionModel.filenameBase} WHERE isUpdated=0`);
  await db.run(`DELETE FROM ${tripUpdatesModel.filenameBase} WHERE isUpdated=0`);
  await db.run(`DELETE FROM ${stopTimesUpdatesModel.filenameBase} WHERE isUpdated=0`);
  await db.run(`DELETE FROM ${serviceAlertsModel.filenameBase} WHERE isUpdated=0`);
  await db.run(`DELETE FROM ${serviceAlertTargetsModel.filenameBase} WHERE isUpdated=0`);
  log(`Cleaned stale GTFS-RT data\r`, true);
}

const updateRtData = async (task) => {

  const model = {
    'vehicle_positions': models.find((x) => x.filenameBase === 'vehicle_positions'),
    'trip_updates': models.find((x) => x.filenameBase === 'trip_updates'),
    'stop_times_updates': models.find((x) => x.filenameBase === 'stop_times_updates'),
    'service_alerts': models.find((x) => x.filenameBase === 'service_alerts'),
    'service_alert_targets': models.find((x) => x.filenameBase === 'service_alert_targets')
  }

  const fields = {
    'vehicle_positions': model.vehicle_positions.schema.map((column) => column.name).join(', '),
    'trip_updates': model.trip_updates.schema.map((column) => column.name).join(', '),
    'stop_times_updates': model.stop_times_updates.schema.map((column) => column.name).join(', '),
    'service_alerts': model.service_alerts.schema.map((column) => column.name).join(', '),
    'service_alert_targets': model.service_alert_targets.schema.map((column) => column.name).join(', ')
  }

  task.log(`Processing ${task.rtupdates.length} sources`);

  for (const rtUpdateFile of task.rtupdates) {
    task.log(`Processing ${rtUpdateFile}`)
    task.log(`Downloading data..`);
    // eslint-disable-next-line no-await-in-loop
    const tripUpdateData = await downloadRTData(rtUpdateFile,task.rtupdate_headers);
    task.log(`Download complete\r`, true);

    task.log(`Processing downloaded data`);
    let totalLineCount = 0;
    for (const entity of tripUpdateData.entity) {

      // Work out which type of stream we got
      let currentModel = null;
      if (entity.vehicle) currentModel = 'vehicle_positions';
      if (entity.tripUpdate) currentModel = 'trip_updates';
      if (entity.alert) currentModel = 'service_alerts';
      if (isNull(currentModel)) break;

      // Do base processing
      const fieldValues = model[currentModel].schema.map((column) => sqlString.escape(getDescendantProp(entity,column.source,column.default)));

      // eslint-disable-next-line no-await-in-loop
      await task.db.run( `REPLACE INTO ${model[currentModel].filenameBase} (${fields[currentModel]}) VALUES (${fieldValues.join(', ')})` ).catch((error)=>{
        task.warn('Import error: '+error.message);
      });  

      // Special procesing of tripUpdates
      if (entity.tripUpdate) {
        const stopUpdateArray = [];
        for (const stopUpdate of entity.tripUpdate.stopTimeUpdate) {
          stopUpdate.parent = entity;
          const subValues = model.stop_times_updates.schema.map((column) => sqlString.escape(getDescendantProp(stopUpdate,column.source,column.default)));
          stopUpdateArray.push(`(${subValues.join(', ')})`);
          totalLineCount++;
        }

        // eslint-disable-next-line no-await-in-loop
        await task.db.run( `REPLACE INTO ${model.stop_times_updates.filenameBase} (${fields.stop_times_updates}) VALUES ${stopUpdateArray.join(', ')}` ).catch((error)=>{
          task.warn('Import error: '+error.message);
        });              
      }

      // Special procesing of serviceAlerts
      if (entity.alert) {
        const alertTargetArray = [];
        for (const informedEntity of entity.alert.informedEntity) {
          informedEntity.parent = entity;
          const subValues = model.service_alert_targets.schema.map((column) => sqlString.escape(getDescendantProp(informedEntity,column.source,column.default)));
          alertTargetArray.push(`(${subValues.join(', ')})`);
          totalLineCount++;
        }

        // eslint-disable-next-line no-await-in-loop
        await task.db.run( `REPLACE INTO ${model.service_alert_targets.filenameBase} (${fields.service_alert_targets}) VALUES ${alertTargetArray.join(', ')}` ).catch((error)=>{
          task.warn('Import error: '+error.message);
        });              
      }
    ...
    task.log(`Processing downloaded data completed`);
  }

  task.log(`All sources processed`);
};
  ...
  mapSeries(
    ...
    (model) =>
      new Promise((resolve, reject) => {
        ...
        // If the model is a database/gtfs-rt model then just silently exit as we dont really care here
        if (model.extension === 'gtfs-rt') {
          resolve();
          return;
        }
...
export async function importGtfs (initialConfig) {
  ...
  await mapSeries(config.agencies, async (agency) => {
    ...
    const task = {
      ...
      rtupdate_headers: agency.rtheaders || false,
      rtupdates: agency.rtupdates || false,
      downloadDir: path,
      path: agency.path,
...
export async function processGtfsRt (initialConfig) {
  const config = setDefaultConfig(initialConfig);
  validateConfigForImport(config);
  const log = _log(config);
  const logError = _logError(config);
  ...
  const agencyCount = config.agencies.length;
  log(
    `Starting GTFS-RT refresh for ${pluralize(
      'agencies',
      agencyCount,
      true
    )} using SQLite database at ${config.sqlitePath}`
  ...
  await cleanStaleRtData(db,log);
  log(`Completed GTFS-RT refresh for ${pluralize('agencies', agencyCount, true)}\n`);
}
