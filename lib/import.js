...
function downloadFiles(task, cb) {
  ...
  .then(buffer => writeFileAsync(task.path, buffer))
  .then(() => {
    cb(null, task);
  })
  .catch(cb);
...
function readFiles(task, cb) {
  ...
  if (path.extname(task.path) === '.zip') {
    ...
    copy(path.join(task.path, '*.txt'), task.downloadDir, err => {
      cb(err, task);
    });
  }
}
...
function removeDatabase(task, cb) {
  ...
  // Override using the --skipDelete command line argument or `skipDelete` in
  // config.json
  if (task.skipDelete) {
