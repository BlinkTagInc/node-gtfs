...
const readFiles = async task => {
  ...
  if (path.extname(gtfsPath) === '.zip') {
    ...
    try {
      ...
      throw new Error(`Unable load files from path \`${gtfsPath}\` defined in configuration. Verify that path exists and contains GTFS files.`);
    }
  }
};
