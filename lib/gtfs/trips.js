...
exports.getTripsByStopId = async (query = {}) => {
  return new Promise(async (resolve, reject) => {
    const stopTimes = StopTime.find({}, utils.defaultProjection, {
      ...
      .where("stop_id")
      .in(query.stops.map(stop => stop.stop_id))
      .then((stopTimes) => {
            ...
            for (var i = 0; i < stopInfo.length; i++) {
              ...
              for (var j = 0; j < tripInfo.length; j++) {
                if (tripInfo[j].trip_id == id) {
                  ...
                  let stop = query.stops.filter(stop => stop.stop_id == stopInfo[i].stop_id )
                  stopInfo[i]["stop_lat"] = stop[0].stop_lat;
                  stopInfo[i]["stop_lon"] = stop[0].stop_lon;
                  break;
                }
              }
            }
