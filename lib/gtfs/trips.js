...
exports.getTripsByStopId = async (query = {}) => {
  ...
  const stopTimes = await StopTime.find({}, utils.defaultProjection, { timeout: true })
    .where('stop_id')
    .in(query.stop_id);

  const trips = await Trip.find({}, utils.defaultProjection, { timeout: true })
    .where('trip_id')
