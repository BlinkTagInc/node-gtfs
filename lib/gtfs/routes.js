...
const getRouteIdsWithinRadius = async query => {
  ...
  const stops = await Stop.find({}, {}, {timeout: true})
    .near('loc', {
      center: [lon, lat],
      spherical: true,
  ...
  const tripIds = await StopTime.find({}, {}, {timeout: true})
    .distinct('trip_id')
 ...
 */
exports.getRoutes = async (query = {}, projection = '-_id -created_at', options = {lean: true, timeout: true}) => {
