...
function stopsToGeoJSON(stops) {
  ...
  .then(results => {
    results.forEach(agency => {
      agencies[agency.agency_key] = agency.toObject();
    });
  })
  .then(() => {
    ...
    return stops.map(stop => stop.toObject());
  })
  ...
  .then(stops => Promise.all(stops.map(stop => {
    ...
    .then(routes => {
      stop.routes = routes.map(item => {
        const route = item.toObject();
        return _.pick(route, ['route_id', 'route_short_name', 'route_long_name']);
      });
      return stop;
    });
  })))
...
exports.getStopsAsGeoJSON = (agencyKey, stopIds, cb) => {
  ...
  .then(stopsToGeoJSON)
  .then(geojson => cb ? cb(null, geojson) : geojson)
  .catch(cb);
};
...
exports.getStopsByRoute = (agencyKey, routeId, directionId, cb) => {
  ...
  .then(results => cb ? cb(null, results) : results)
  .catch(cb);
};
