...
exports.getStopsByRoute = (agencyKey, routeId, directionId) => {
  ...
  .then(trips => {
    return trips.reduce((memo, trip) => {
      if (!memo[trip.direction_id]) {
        memo[trip.direction_id] = [];
      }

      memo[trip.direction_id].push(trip.trip_id);
      return memo;
    }, {});
  })
  .then(groupedTrips => Promise.all(_.map(groupedTrips, (tripIds, directionId) => {
    results.push({direction_id: directionId});

    return Promise.all(tripIds.map(tripId => {
