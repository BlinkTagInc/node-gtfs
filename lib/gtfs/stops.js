...
exports.getStops = async (query = {}, projection = '-_id', options = {lean: true}) => {
  ...
  if (query.stop_id.$in) {
    const unsortedStops = await Stop.find(query, projection, options);
    const sortedStops = unsortedStops.sort(function(a, b) {
      return query.stop_id.$in.indexOf(a.stop_id) - query.stop_id.$in.indexOf(b.stop_id); 
    });
    return sortedStops;
  } else {
    return Stop.find(query, projection, options);
