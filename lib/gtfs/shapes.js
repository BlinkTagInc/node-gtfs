 ...
 */
exports.getShapes = async (query = {}, projection = '-_id', options = {lean: true, sort: {shape_pt_sequence: 1}, timeout: true}) => {
  const shapes = [];
  let shapeIds;
...
exports.getShapesAsGeoJSON = async (query = {}) => {
  ...
  const routes = await Route.find(routeQuery, '-_id', {timeout: true}).lean();
  const features = [];
