...
function shapesToGeoJSON(shapes, routeId) {
  return {
    ...
    features: shapes.map(shape => ({
      ...
      properties: {
        route_id: routeId,
        shape_id: shape && shape.length ? shape[0].shape_id : null,
        agency_key: shape && shape.length ? shape[0].agency_key : null
      }
    }))
  };
}
...
exports.getShapesByRouteAsGeoJSON = (agencyKey, routeId, directionId, serviceIds, cb) => {
  ...
  .then(shapes => {
    return shapesToGeoJSON(shapes, routeId);
  })
  .then(geojson => {
    if (cb) {
      cb(null, geojson);
