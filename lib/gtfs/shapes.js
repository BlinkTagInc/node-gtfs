 ...
 */
exports.getShapes = async (query = {}, projection = '-_id -created_at', options = {lean: true, sort: {shape_pt_sequence: 1}, timeout: true}) => {
  const shapes = [];
  let shapeIds;
  ...
  if (_.isString(query.shape_id)) {
    ...
    shapeIds = await Trip.find(tripQuery, {}, {timeout: true}).distinct('shape_id');
  }
...
exports.getShapesAsGeoJSON = async (query = {}) => {
  ...
  const routes = await Route.find(routeQuery, '-_id -created_at', {timeout: true}).lean();
  const features = [];
