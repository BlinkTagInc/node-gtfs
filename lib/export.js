...
module.exports = async config => {
  ...
  log(`Starting GTFS export for ${pluralize('agency', agencyCount, true)}`);
  ...
  const exportedFiles = await Promise.mapSeries(models, async model => {
    ...
    if (!lines || lines.length === 0) {
      if (!model.nonstandard) {
        log(`${agencyKey}: Skipping (no data) - ${model.filenameBase}.txt\r`);
      }
    ...
    log(`${agencyKey}: Exporting - ${model.filenameBase}.txt\r`);
