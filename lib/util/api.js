    ...
    StopTime
      ...
      .run(function(err, results){
        if(results){
          ...
          callback(new Error('No trips to any stops within ' + radius + ' miles'), 'trips');
        }
      });
  }
    ...
    Trip
      ...
      .run(function(err, results){
        if(results){
          ...
          callback(new Error('No routes to any stops within ' + radius + ' miles'), 'routes');
        }
      });
  }
  
  function lookupRoutes(callback){
    Route
    .find()
    ...
    .run(function(err, results){
      if(results){
        ...
        callback(new Error('No information for routes'), 'lookup');
      }
    });
  }
