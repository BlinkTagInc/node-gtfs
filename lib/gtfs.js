...
module.exports = {    
  ...
  getAgenciesByDistance: function(lat, lon, radius, cb){
    ...
    if (_.isFunction(radius)) {
      cb = radius;
      radius = 0.25; // default is ~ 1/4 mile
    }
  ...
  getRoutesByDistance: function(lat, lon, radius, cb){
    ...
    async.series([
      ...
      cb(e, (routes.length) ? routes : null);
    });
...
/*
    ...
    getStopsByRoute: function(req, res){
      ...
      var agency_key = req.params.agency
        , route_id = (parseFloat(req.params.route_id)) ? parseFloat(req.params.route_id) : req.params.route_id
      ...
      function getTrips(cb){
        ...
        Trip.find({agency_key: agency_key}).run(function(e, result){ console.log(result) });  
